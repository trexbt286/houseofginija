...
settings.jewellery_enabled = 'true';
fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2), 'utf8');
console.log('Successfully set jewellery_enabled = true in local-settings.json!');
  ...
  async function main() {
    try {
      ...
      await client.query(`
        ...
        VALUES ('jewellery_enabled', 'true')
        ON CONFLICT (key) DO UPDATE SET value = 'true'
      `);
      console.log('CockroachDB settings table updated: jewellery_enabled = true!');
      client.release();
    } catch (err) {
