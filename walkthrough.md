...
5. **Admin Portal Mobile Responsiveness Refinements**
   - **Top Navigation Bar:** Replaced the tall sidebar with a streamlined horizontal top navigation bar under 768px. Condensed the labels to single words (Dashboard, Products, Flash Sale, Orders, Coupons) and dynamically scaled down their font size so all 5 options fit seamlessly on a single row without horizontal scrolling or truncation.
   - **Streamlined Header:** Changed "Ginija Portal" and "Management System" into a unified "Admin Portal" title on mobile. Moved the "Visit Storefront" and "Sign Out" global action buttons directly into the same header row, perfectly aligned to the right. 
   - **Column-Selective Tables:** Completely eliminated horizontal scrolling from all tables by setting `display: table` and hiding non-essential telemetry on mobile. Using `.hide-on-mobile`, the system now automatically hides secondary columns (like Slug, Category, Stock, Payment Status, etc.) and focuses exclusively on displaying the 3 most critical columns for each page (e.g. Product Details, Price, Actions).
